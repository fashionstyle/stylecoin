...
void StakeMiner(CWallet *pwallet)
{
    ...
    RenameThread("stylecoin-stakeminer");
...
void static StylecoinMiner(CWallet *pwallet)
{
    printf("StylecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    ...
    try { loop {
        ...
        {
            printf("StylecoinMiner : no more PoW blocks\n");
            return;
        }

        IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

        printf("Running StylecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("StylecoinMiner terminated\n");
        throw;
    }
}

void GenerateStylecoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&StylecoinMiner, pwallet));
}
