...
#if defined(NDEBUG)
# error "Stylecoin cannot be compiled without assertions."
#endif
...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Stylecoin: starting difficulty is 1 / 2^12
...
{
    // Stylecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
const int64 nTargetSpacing = 60; // Stylecoin: 1 minute block
static const int64 nTargetTimespan = 60 * 60; // Stylecoin: Not used
...
void ThreadScriptCheck() {
    RenameThread("stylecoin-scriptch");
...
{
    // Stylecoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Stylecoin: increase each by adding 2 to bitcoin's value.
